...
def check_request(call):
    ...
    if call.data.startswith("ok_"):
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("Akkaunt sotib olish 💬", callback_data=f"buy_{user_id}"))

        if photos:
            bot.send_photo(CHANNEL_ID, photos[0], caption=caption, parse_mode="Markdown", reply_markup=markup)
            for p in photos[1:]:
...
# 🔹 Sotib olish tugmasi (kanaldagi)
@bot.callback_query_handler(func=lambda c: c.data.startswith("buy_"))
def buy_request(call):
    seller_id = int(call.data.split("_")[1])
    info = user_data.get(seller_id)
    buyer_id = call.message.chat.id

    if not info:
        bot.answer_callback_query(call.id, "❌ Akkaunt topilmadi.")
        return

    # Adminga xabar — rasm + ma’lumot bilan
    caption = (
        f"🛒 *Akkaunt sotib olish so‘rovi*\n\n"
        f"👤 Xaridor: [{call.from_user.first_name}](tg://user?id={buyer_id})\n"
        f"🔗 {info['link']}\n"
        f"💰 Narxi: {info['price']} so‘m\n"
        f"👥 Obunachilar: {info['followers']}\n"
        f"⭐ Afzalliklar: {info['features']}"
    ...
    if info['photos']:
        bot.send_photo(ADMIN_ID, info['photos'][0], caption=caption, parse_mode="Markdown")
    else:
        bot.send_message(ADMIN_ID, caption, parse_mode="Markdown")

    # Xaridorga xabar
    bot.send_message(buyer_id, "⏳ Akkaunt sotib olish uchun kuting, admin tez orada siz bilan bog‘lanadi.")
