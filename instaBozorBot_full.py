...
def get_link(message):
    user_data = {'link': message.text, 'user_id': message.from_user.id, 'username': message.from_user.username or message.from_user.first_name}
...
def get_photos(message, user_data, photos):
    if message.content_type == 'photo':
        ...
        if len(photos) < 5:
            ...
            return
    elif message.text and message.text.lower() == 'tugatdim':
        if len(photos) == 0:
            bot.send_message(message.chat.id, "Kamida bitta rasm yuboring.")
            bot.register_next_step_handler(message, get_photos, user_data, photos)
            return
    else:
        bot.send_message(message.chat.id, "Faqat rasm yuboring yoki 'tugatdim' deb yozing:")
        bot.register_next_step_handler(message, get_photos, user_data, photos)
        return
...
def get_advantages(message, user_data):
    ...
    bot.send_message(message.chat.id, "✅ Ma’lumotlar adminga yuborildi, 24 soat ichida kanalga joylanadi.")
    ...
    text = (
        ...
        f"👤 Foydalanuvchi: @{user_data['username']}"
    )

    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("✅ To‘g‘ri", callback_data=f"ok|{user_data['user_id']}"),
        types.InlineKeyboardButton("❌ Noto‘g‘ri", callback_data=f"no|{user_data['user_id']}")
    )
...
def callback_handler(call):
    ...
    user_id = int(user_id)
    if action == "ok":
        bot.send_message(user_id, "✅ Akkauntingiz tekshirildi va kanalga joylandi.")
        bot.send_message(
            CHANNEL_ID,
            f"🆕 Yangi akkaunt sotuvga qo‘shildi! 📌 Kanalimizdan ko‘ring."
        )
    else:
        bot.send_message(user_id, "❌ Ma’lumotlar xato. Iltimos, qayta urinib ko‘ring.")
